...
export function CategoryChart({ data, total }: { data: CategorySpending[]; total: number }) {
  ...
  const visible = otherAmount > 0 ? [...data.slice(0, 6), { id: 'other-categories', name: 'Outros', color: '#64748b', amount: otherAmount, percentage: total > 0 ? (otherAmount / total) * 100 : 0 }] : data.slice(0, 6)
