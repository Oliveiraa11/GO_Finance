...
export function CreateBudgetDrawer({ open, categories, budgets, onClose, onCreate }: CreateBudgetDrawerProps) {
  const titleId = useId()
  ...
  const availableCategories = useMemo(() => categories.filter((category) => !budgets.some((budget) => budget.month === month && budget.categoryId === category.id)), [budgets, categories, month])
