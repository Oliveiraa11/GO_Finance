...
import { accounts, categories } from '../../data/mocks'
  ...
  return <form onSubmit={submit} className="flex min-h-full flex-col">
    <div className="flex-1 space-y-5">
      ...
      <div className="flex gap-1 rounded-xl bg-canvas p-1">
        <button type="button" onClick={() => setKind('expense')} className={`flex h-9 flex-1 items-center justify-center gap-1.5 rounded-control text-sm font-semibold transition ${kind === 'expense' ? 'bg-danger/15 text-red-300' : 'text-muted'}`}><ArrowUp size={16} />Despesa</button>
        <button type="button" onClick={() => setKind('income')} className={`flex h-9 flex-1 items-center justify-center gap-1.5 rounded-control text-sm font-semibold transition ${kind === 'income' ? 'bg-primary/15 text-primary' : 'text-muted'}`}><ArrowDown size={16} />Receita</button>
      </div>
      ...
      <div className="space-y-3">
        ...
        <Field label="Categoria"><select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="control"><option value="">Selecionar categoria</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></Field>
        <div className="grid grid-cols-2 gap-3"><Field label="Data"><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="control" /></Field><Field label="Conta / pagamento"><select value={accountId} onChange={(e) => setAccountId(e.target.value)} className="control">{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}<option value="other">Outro</option></select></Field></div>
