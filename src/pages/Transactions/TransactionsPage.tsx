...
import { accounts, categories, transactions as initialTransactions } from '../../data/mocks'
...
export function TransactionsPage() {
  ...
  useEffect(() => {
    const openNewTransaction = () => { setEditing(null); setDrawerOpen(true) }
    window.addEventListener('go-finance:open-transaction-drawer', openNewTransaction)
    return () => window.removeEventListener('go-finance:open-transaction-drawer', openNewTransaction)
  }, [])
  ...
  function save(draft: TransactionDraft) {
    if (editing) setTransactions((current) => current.map((item) => item.id === editing.id ? { ...draft, id: editing.id } : item))
    else setTransactions((current) => [{ ...draft, id: crypto.randomUUID() }, ...current])
    closeDrawer()
  }
  function edit(transaction: Transaction) { setEditing(transaction); setDrawerOpen(true); setOpenMenu(null) }
  function duplicate(transaction: Transaction) { setTransactions((current) => [{ ...transaction, id: crypto.randomUUID(), description: `${transaction.description} (cópia)` }, ...current]); setOpenMenu(null); setPage(1) }
  function remove(id: string) { setTransactions((current) => current.filter((item) => item.id !== id)); setOpenMenu(null) }

  return <div className="space-y-4">
    <header className="flex items-center justify-between"><div><h1 className="text-[28px] font-semibold tracking-tight">Transações</h1><p className="mt-1 text-sm text-muted">Acompanhe todas as suas movimentações financeiras com precisão.</p></div><Button onClick={() => setDrawerOpen(true)}><Plus size={17} />Nova transação</Button></header>
    ...
    <section className="flex items-center gap-3 rounded-card bg-surface p-3">
      ...
      <Filter value={account} onChange={(value) => { setAccount(value); setPage(1) }}><option value="all">Todas as contas</option>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Filter>
    ...
    <section className="overflow-visible rounded-card bg-surface">
      ...
      <div>{visible.length ? visible.map((transaction) => {
        ...
        const accountItem = accounts.find((item) => item.id === transaction.accountId)
    ...
    {drawerOpen && <div className="fixed inset-0 z-50"><button aria-label="Fechar drawer" onClick={closeDrawer} className="absolute inset-0 h-full w-full cursor-default bg-black/70 backdrop-blur-sm" /><aside role="dialog" aria-modal="true" aria-label={editing ? 'Editar transação' : 'Nova transação'} className="absolute bottom-0 right-0 top-0 w-[440px] max-w-full overflow-y-auto bg-[#181d22] p-6 shadow-2xl"><TransactionForm key={editing?.id ?? 'new'} transaction={editing} onCancel={closeDrawer} onSubmit={save} /></aside></div>}
  </div>
}
